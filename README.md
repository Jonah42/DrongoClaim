...
Currently DrongoClaim only checks that your zID and names are valid. It's up to you to ensure that you've provided valid dates and times (it won't submit but will crash which isn't great - currently working on better data checking)
...
Note that DrongoClaim uses a ChromeDriver in order to control Chrome. This ChromeDriver is specific to a version of Chrome. You can find out your version of Chrome by going to `chrome://version/` in Chrome. If for some reason you need to use a different version of the ChromeDriver, you can find all versions here `https://chromedriver.chromium.org/downloads`. Note that the ChromeDriver is different for Mac and Windows, it can be found under `driver/Mac` or `driver/Windows`. DrongoClaim automatically detects which OS you are on and uses the appropriate driver. Hopefully the existing driver in the repo will work for you though.
...
At this point the best way to work out what is going wrong is to email me with the error message from the terminal and any relevant screenshots/context. Better error handling coming soon!
