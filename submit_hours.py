...
def submit(data):
    ...
    if sys.platform.startswith('win32'):
        ...
        browser = webdriver.Chrome('./driver/darwin/chromedriver', options=options)
    elif sys.platform.startswith('linux'):
        browser = webdriver.Chrome('./driver/linux/chromedriver', options=options)
    else:
        raise Exception(f'OS {sys.platform} not supported (ChromeDriver not found)')
            ...
            for course in reversed(data['courses']):
                while (len(browser.find_elements_by_css_selector('input')) != 3):
                    if (len(browser.find_elements_by_css_selector('input')) != 31):
                        ...
                        time.sleep(1)
                submitCourse(browser, data['zid'], data['firstName'], data['lastName'], course, submittedTimes)
        except Exception as e:
            pickle.dump(submittedTimes, f)
            browser.quit()
...
def submitCourse(browser, zid, firstName, lastName, course, submittedTimes):
    for slot in reversed(course['slots']):
        ...
        # Save a record that we submitted this timeslot
        submittedTimes.append(slot['date']+slot['month']+slot['startHour']+slot['startMin']+slot['endHour']+slot['endMin'])
